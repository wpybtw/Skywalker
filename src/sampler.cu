...
__device__ void SampleWarpCentic(sample_result &result, gpu_graph *ggraph, curandState state, int current_itr, int idx, int node_id)
{
...
__device__ void SampleBlockCentic(sample_result &result, gpu_graph *ggraph, curandState state, int current_itr, int idx, int node_id)
{
  __shared__ alias_table_shmem<uint> tables[1];
  alias_table_shmem<uint> *table = &tables[0];
