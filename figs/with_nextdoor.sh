...
#     for i in $(seq 1  ${ITR})
#     do
#         ./bin/main -bias=1 --ol=0 ${SG} ${RW} --input ~/data/${DATA[idx-1]}${GR} --hd=${HD[idx-1]} ${BATCH} --tp=0.15 >> ${LOG_FILE} 
#     done
# done


echo "-------------------------------------------------------unbiased sp 100" >> ${LOG_FILE}
for idx in $(seq 1 ${#DATA[*]}) 
do
    for i in $(seq 1  ${ITR})
    do
        ./bin/main -bias=0 --ol=0 ${SG} ${SP} --input ~/data/${DATA[idx-1]}${GR} --hd=${HD[idx-1]} ${BATCH} >> ${LOG_FILE}
    done
done
