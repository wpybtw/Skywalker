...
###
 ...
 # @LastEditTime: 2021-01-15 15:16:59
 # @FilePath: /skywalker/figs/test copy 2.sh
### 
...
do
    ./bin/main --bias=0  --input ~/data/${DATA[idx-1]}${GR}  --ngpu 1 ${RW} ${BATCH}
done

echo "-------------------------------------------------------unbias sp"
for idx in $(seq 1 ${#DATA[*]}) 
do
    ./bin/main --bias=0  --input ~/data/${DATA[idx-1]}${GR}  --ngpu 1 ${SP} ${BATCH}
done
